...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Memanggil API Login dengan credentials 'include' agar JWT cookie tersimpan
      ...
      // Simpan role di localStorage untuk UI state, Cookie mengurus keamanan API
      localStorage.setItem('role', data.user.role);

      // Cek Role Database untuk Routing
    ...
    <div className="min-h-screen w-full bg-stone-950 flex items-center justify-center p-4">
      ...
      <div className='card w-full max-w-5xl flex flex-col md:flex-row min-h-[650px] md:h-[650px] rounded-2xl overflow-hidden shadow-2xl border border-stone-800 relative bg-stone-900'>
        <div className='w-full md:w-1/2 h-64 md:h-full overflow-hidden bg-stone-950 relative z-10 border-b md:border-b-0 md:border-r border-stone-800'>
          <img src='https://images.unsplash.com/photo-1510312305653-8ed496efae75?q=80&w=1280&auto=format&fit=crop' alt="Scenic mountain campground" className="w-full h-full object-cover transition-transform duration-700 opacity-70 hover:opacity-100 hover:scale-105" />
        </div>
        ...
        <div className='w-full md:w-1/2 px-8 py-10 md:px-12 flex flex-col justify-center relative overflow-hidden z-0' onMouseMove={handleMouseMove} onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)}>
          ...
          <div className="z-10 relative flex flex-col justify-center">
            <form className='text-center grid gap-2' onSubmit={handleLogin}>
              <div className='grid gap-4 mb-4 relative z-10'>
                <h1 className='text-4xl font-extrabold text-white tracking-tight'>Selamat Datang</h1>
                <div className="flex items-center justify-center mt-2 relative z-10">
                  <ul className="flex gap-4">
                    {socialIcons.map((social, idx) => (
                      <li key={idx} className="list-none">
                        <a href={social.href} className="w-12 h-12 bg-stone-950 rounded-full flex justify-center items-center relative z-[1] border border-stone-800 overflow-hidden group">
              ...
              <div className='grid gap-5 items-center mb-6 mt-2 relative z-10'>
                <input placeholder="Email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-4 rounded-xl bg-stone-950 border border-stone-800 text-white focus:border-emerald-500 outline-none transition" />
                <div className="relative">
                  <input placeholder="Password" type={showPassword ? "text" : "password"} required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-4 rounded-xl bg-stone-950 border border-stone-800 text-white focus:border-emerald-500 outline-none transition pr-12" />
              ...
              <div className='flex justify-center relative z-10'>
                <button type="submit" disabled={isLoading} className="group relative inline-flex w-full justify-center items-center overflow-hidden rounded-xl bg-stone-800 border border-stone-700 px-8 py-3.5 text-sm font-bold text-white hover:border-emerald-500 disabled:opacity-50 cursor-pointer">
                  <span className="relative z-10 text-emerald-400 group-hover:text-white transition-colors">{isLoading ? "Memeriksa..." : "Masuk"}</span>
              ...
              <div className="text-sm text-stone-400 mt-6 z-10 relative">Belum punya akun? <Link href="/register" className="text-emerald-400 font-bold hover:underline">Daftar di sini</Link></div>
            </form>
          </div>
        </div>
      </div>
